	...
	public void actionPerformed(ActionEvent evt)
	{
		try
		{
			if(evt.getSource()==btn) {
				...
				String sql = "INSERT INTO USERSECTION VALUES('" + s1 + "','" + s2 + "','" + s3 + "','" + s4 + "','" + s5 + "');";
				String sql1 = "INSERT INTO USERSIGNIN VALUES('" + s4 + "','" + s5 + "');";
				st.executeUpdate(sql);
				st.executeUpdate(sql1);

				if (st.executeUpdate(sql) == 1 && st.executeUpdate(sql1) == 1){
					JOptionPane.showMessageDialog(null, "You are now registered, Please SignIN");
					new Home();
				}
			}
		}
